...
and a pre-freeze calibration (`scripts/calibrate_encoder_study.py`, committed with this document)
that reads **no target** (no apple position, no expert command) and fits **no readout**: it
renders frames and measures encoder activations. **The test split is never decoded. Cohort D
...
whose features pass TASK-061's L-hypothesis probe on the same post-look 112 px frames, with the
same readouts and the unchanged TASK-059 bars, on roots the encoder never saw, and do better
than a random-init encoder of the same architecture?
...
**What changes:** the features (§5), the rule for which encoder produces a root's features (§4),
the multiplicity family (§7) and the floor as a pass condition (§6). The bars do not change.

## 3. Why might E0 lose the apple? The explanations and a cheap test

### 3.1 What E0's probe feature is
...
**CLS token → projector MLP (BatchNorm) → per-camera linear fusion → a 128-d vector.**

That vector is the world model's latent. E0 was trained with the TASK-054 recipe: one-step and
multistep latent prediction, SIGReg at weight 0.09, proprioception fused into the latent
(`state_fusion`), and privileged readout heads.

### 3.2 Candidate explanations

| id | explanation | what it predicts |
|---|---|---|
| **E-pool** | The apple survives in the spatial tokens. The single CLS token and the projector pool it away. | Token features read the apple, and the pooled feature does not. |
| **E-collapse** | The latent sits in a low-rank regime (weak SIGReg). Its few effective dimensions go to the arm and the plate. | The pooled feature has a low effective rank on post-look frames, and stronger anti-collapse regularisation helps. |
| **E-objective** | Latent prediction gives no incentive to keep a small, static object. The apple does not move until grasped, and the arm dominates frame-to-frame change. | A term that makes the latent keep image content (pixel reconstruction) helps. |
| E-capacity (not tested) | 128 dimensions are too few. | Refuted in advance by §3.3: E0 uses about 3 effective dimensions of 128 on these frames. |
| E-input (not tested) | Patch 14 at 112 px is too coarse for a median 12 px apple. | Refuted in advance by §3.3: E0's patch embedding and tokens respond to the apple. TASK-061's resolution arm (LH) added only 0.084 cm once the wrist was clear. |
| E-distribution (not tested) | The post-look pose is out of E0's training distribution. | Not separable here. The look is the expert's own step-0 command without its reset-dependent `dx`, so the post-look pose lies close to the expert path that the corpus covers. |

The evidence already in hand: E0 is worse than a random init of its own architecture (§1). So
**training** moved the pooled feature away from the apple. That points at the objective or the
regime (E-objective, E-collapse), or at training that routed the apple out of the pooled path
(E-pool). It does not point at capacity or input.
...
For each representation r it reports two numbers:
...
It reads no target and fits nothing. (It also reports R_apple, an energy ratio; see the caveat
below.) The artifact is `outputs/task062-encoder-study/calibration-v1.json`, sha256 in the
manifest. It runs in 31 s on CPU. Selected rows:
...
- In E0 the plate dominates every stage after the patch embedding more than it does in raw pixels
  (S_plate about 3–4, against 1.5).
- This is consistent with E-pool and E-collapse together. The loss happens between the tokens and
  the latent, and the latent is low-rank on these frames.
...
measures **readability**. That is what the probe measures, which is why the explanation is tested
in the run (§5.3, E0-tok) and not assumed.
...
- **Features.** A root in fold k is always featurised by the encoder that excluded k's half.
  - The outer readout for fold k is trained and selected on the other 171 roots, with the
    features of that same encoder.
  - Those training roots include roots the encoder saw. The **held-out** fold never does.
  - The readout is therefore fitted on a mix of seen and unseen roots and tested on unseen ones.
    Any mismatch between the two works against the arm, never for it.
- **Training data.** A training root contributes its root episode and its 3 branch episodes
  (grouped by `root_episode_id`, 4 episodes per root). That comes to about 85 roots, 340 episodes
  and 88 000 frames per half.
...
Every model starts from **the same seed-0 initialisation**, and that initialisation is the random
floor (§6). Training runs on **MPS**. Feature extraction and all readouts run on **CPU**,
`eval()`, `no_grad`.

### 5.2 The three decisional arms
...
| **A-sig** | E-collapse | `sigreg_weight` **1.0** (E0: 0.09); one existing config key | the probe feature (as TASK-061) | **F-cls** |
...
- **A-tok** needs no model of its own. It reads R0's tokens, so it isolates the readout point with
  the objective fixed.
- **A-sig and A-rec** keep TASK-061's feature, which is the world model's latent. A pass there
  would be a drop-in LeWM encoder.
- **Single values, no sweep.** 1.0 is about 11× E0's SIGReg weight. The reconstruction weight of
  1.0 puts the pixel loss (starting near 1) on the same scale as the other terms.

**A-rec's decoder** (outside `VisualModel`: no hashed model file changes, so E0 still loads under
its implementation hash):

- Linear 128 → 256·7·7, then four ConvTranspose2d stages (kernel 4, stride 2, padding 1) with
  channels 256→128→64→32→3 and GELU between them: 7 → 112 px.
- It is initialised under `torch.manual_seed(6210)`, inside a forked RNG, so the model's own
  initialisation is untouched.
- The loss is the mean squared error on the **first frame of each window** (32 frames per step).
- The decoder has its own AdamW with the same learning rate, schedule and weight decay. Gradient
  clipping at 1.0 covers the model and decoder parameters jointly.
- The A-rec step is the LeWM training step plus this term. PR 2 must test that with the term at
  weight 0 it equals `LeWM.train_step` exactly.
...
| **F-cls** = L-random | seed-0 random init, probe feature | **floor for A-sig and A-rec**; also a reproduction anchor |
...
- If E0-tok succeeds while L-E0 does not, E-pool is supported on E0 itself (with the
  held-out caveat).
- If R0-cls succeeds, E0's failure did not reproduce under cross-fitting, and the arm comparisons
  are read against R0-cls rather than E0.
...
- **F-cls is TASK-061's L-random** (the same seed-0 model, a1's digest). It reproduces its
  TASK-061 numbers bit for bit (§9): T1 1.124 cm, ratio upper bound 0.711, T2 176/190. To beat it,
  an arm needs a median error clearly below it **and** at least 177/190 on T2.
- **F-tok is new.** The tokens are a random linear-plus-attention map of the patches, so F-tok may
...
1. **Trained.** Both of its encoders complete 15 000 steps within the per-encoder cap, with finite
   losses (§13).
2. **Not collapsed.** On 1024 frames sampled from each encoder's own training half, the image
   feature (the probe feature, for every arm) has:
   - `collapsed_fraction` ≤ 0.05;
   - `effective_rank` ≥ 2;
   - `latent_std_mean` ≥ 0.1.

   These are TASK-054's selection-eligibility values, computed with `VisualModel`'s own
   `_statistics`.
...
5. **Holm rejection** at a family-wise one-sided α = 0.025 over the three arms.
   ...
   - p_arm = 1 if a point condition fails: T1 median > 1.5 cm, T2 accuracy < 0.85, or T2 accuracy
     not higher than the floor's.
   - The thresholds are 0.00833, 0.0125 and 0.025. Ties go to the listed order: A-tok, A-sig,
     A-rec.
...
- **Collapse**, for every trained encoder:
  - `_statistics` of the probe feature and of the flattened final tokens, on 1024 training-half
    frames (seed 6220);
...
- **Action sensitivity**, for every trained world model, on 512 training-half windows of horizon 8
  (seed 6230). The statistic is the median over windows of
  ‖ẑ₈(a) − ẑ₈(a_π)‖ / ‖ẑ₈(a) − z₈‖, where ẑ₈ is the 8-step rollout, a_π the actions of another
  window (a seeded derangement) and z₈ the encoded target. A world model that ignores its actions
  scores near 0.
- **The label-free table of §3.3**, recomputed for every trained encoder, on its own held-out
  roots.
...
- **Warm-up rule:** the frames come from TASK-061's own `measure`, which warms up every renderer
  (one discarded render) before any frame is kept.
...
| **O-ENC-LATENT** | A-sig or A-rec passes | A changed training objective or regime gives LeWM's own 128-d latent the apple, on held-out roots, beyond its random init. | **Preregister the look-prefix 112 px corpus with that arm's recipe as the encoder.** If both arms pass, the one with the smaller p_arm is used (ties: A-sig). No control formulation is preregistered until the encoder, retrained on that corpus, passes this probe on its decision frames. |
| **O-ENC-TOKENS** | A-tok passes, and neither A-sig nor A-rec does | The recipe's tokens carry the apple beyond random init. Its pooled latent does not. | **Preregister the look-prefix corpus, plus, separately preregistered, a LeWM variant whose latent is the token grid (or a pooling that keeps it).** The pooled 128-d latent is not used for the apple. |
| **O-ENC-ARCH** | no arm passes; at least one arm **succeeds** (bars) and is not spurious | Features of this architecture expose the apple, but the gain cannot be credited to training: the floor or Holm fails. | **The abandonment clause fires** (below). Recommended next: one preregistered test of an externally pretrained frozen encoder against its own random-init floor, on this probe, before any corpus. |
| **O-ENC-NONE** | no arm succeeds, or every succeeding arm is spurious | No tested change to the training objective, regime or readout point gives a frozen encoder that exposes the apple on held-out roots. | **The abandonment clause fires.** Recommended next: as for O-ENC-ARCH. If that test also fails, no look-prefix corpus is collected, and the remaining route is the observation change TASK-061 already validated: the `overview` camera, a **hardware/workspace change** on the robot. |

**Abandonment clause (for O-ENC-ARCH and O-ENC-NONE).** The line *"train a LeWM-family encoder on
`apple-wide-v1` train-split frames so that its frozen features expose the post-look apple"* is
closed. No further objective, regularisation, pooling, capacity, augmentation or input-handling
variant of it is preregistered on this corpus without new evidence of a different kind. The LeWM
backend, the encoder as a component, and the product goal are **not** abandoned by this clause.

**Always reported, whatever the row:**
- every arm's numbers, p-values, Holm step, floor comparison, spurious check and collapse
  diagnostics;
- every reported-only source (§5.3) on every stratum, and the secondary estimate;
- the paired comparisons of each arm with its floor, with L-raw, with R0-cls and (A-tok only) with
  E0-tok, and of R0-cls with L-E0;
...
| guard | condition |
|---|---|
| **G-hash** | Every file in the manifest's `hashes` matches. That covers TASK-061's runner and module, TASK-059's runner and `info_ceiling.py`, TASK-061's manifest and run-1 report, this study's calibration script, the E0 checkpoint and the task056 a1/a2 checkpoints, the configs, the dataset manifest, `models/base.py`, `models/lewm.py` and `policy.py`. Every episode file is checked against the dataset manifest's sha256 before it is decoded. Encoder digests (E0, random) match TASK-061's. The tree is clean. |
| **G-split** | TASK-061's G-split: exactly the 190 train + val roots and the fold hash. In addition, every episode decoded for training must: (a) be in the dataset's train split; (b) have its `root_episode_id` among the train roots of the encoder's training half; (c) have no root in the half the encoder will featurise. The training root sets of e(A) and e(B) are disjoint, and together they are exactly the 170 train roots. No val, test or holdout episode is ever decoded for training. |
| **G-render, G-expert, G-look, G-prior** | TASK-061's, unchanged, run by TASK-061's `measure` and checks. |
| **G-anchor** | §9. |

**Not guards.** An arm-level failure demotes that arm, and the run continues:
- a training budget stop;
- a non-finite loss or gradient in one arm;
- a failed collapse gate.

Any other exception is a crash, and a crash is V.

**PR 2's tests** must exercise, in both directions:
- every guard, including G-anchor's three branches and G-split's half-disjointness;
- the cross-fitted nested CV. With identical features for both halves it must equal
  `info_ceiling.nested_cv` exactly. Each held-out fold's features must come from the encoder that
  excluded it;
- the A-rec step: at weight 0 it equals `LeWM.train_step`;
- p_F, the floor condition and the Holm step-down over three arms;
- the spurious check with per-half hidden features;
- arm demotion (budget, non-finite loss, collapse), without voiding the run;
...
- **Exactly one from-scratch repeat** is allowed, into `run-2`: the same seeds, caps and device,
  and a clean tree. Every encoder is retrained.
  - MPS training is not bit-deterministic, so the repeat's weights will differ. Their digests are
    recorded.
...
- **Caps:**
  - **7200 s per encoder.** E0's full-data run took 3512 s including 16 validation passes. An
    encoder that stops at the cap fails its arm; that is not a void.
  - **15 h global wall cap** for the whole run; exceeding it is V.
  - Expected: about 6 encoders × 1 h, plus about 30 min for frames, anchors and readouts.
- **Seeds:**
  - model initialisation 0 (every arm, both halves, and the floors);
  - window samplers 6200 (e(A)) and 6201 (e(B));
  - the A-rec decoder 6210;
  - collapse sample 6220; action-sensitivity sample 6230;
...
- **Recorded:**
  ...
  - per-encoder weight digests, training curves (every 25 steps), elapsed time and completion;
...
uv run --no-sync python scripts/calibrate_encoder_study.py \
    --output outputs/task062-encoder-study/calibration-v1.json
```

- **What it reads.** The 190 roots of the frozen plan, checked against the dataset's splits. It
  renders their post-look frames through TASK-061's `make_robot`, warm-up and look. It builds E0
  and the random init through TASK-061's encoder loader, and checks both digests.
- **What it does not read.** It opens no episode file for frames, and it reads no apple position,
  no expert command and no label. The ablation renderer reproduces the post-look frame on 190/190
  roots.
- **Superseded draft.** `calibration-draft-1.json` (sha256 in the manifest) differs only in a
  mislabelled statistic. It named the energy ratio a "variance fraction", although it can exceed 1,
  and it divided by 2n. The renaming happened before this document was written. Every S value and
  every effective rank is identical between the two files.
...
- No change is made to `models/`, `policy.py`, `simulation.py`, `info_ceiling.py`,
  `observation_reprobe.py`, TASK-061's runner or any checkpoint. The A-rec decoder and the
  cross-fitting live in new code.
- **No encoder is trained on post-look frames.** The training data is the existing train-split
  corpus. Adding re-rendered look sequences would need privileged labels for frames the corpus
  does not have. That belongs to the corpus task.
...
- No 4th arm. Capacity and input handling are argued out in advance (§3.2). Augmentation has no
  specific support in the TASK-061 evidence.
...
2. **PR 2:** the runner, the cross-fitting and A-rec code, and the tests of §11. It merges on a
   reported APPROVE and green CI.
