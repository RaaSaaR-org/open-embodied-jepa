...
- `predict`: actions float32 `[B, K, T, A]` for K candidate sequences and T future actions. The adapter handles broadcasting/tiling the encoded state and returns opaque predicted states for all T steps. Goal-image latents need not contain proprioception; `distance` must compare features available from the image goal internally. A documented learned cross-modal adapter may infer pose features from RGB using TRAIN-only supervision; it may not accept a supplied goal robot state.
...
## Frozen visual/pose goal composition (TASK-042)
